...
export function SignInForm({ providers = ['github'], className }) {
    ...
    return (_jsxs("div", { className: className || '', children: [_jsxs("div", { className: "flex gap-1 p-1 bg-zinc-100 rounded-xl mb-6", children: [_jsx("button", { type: "button", className: tabClass('signin'), onClick: () => switchTab('signin'), children: "Sign In" }), _jsx("button", { type: "button", className: tabClass('signup'), onClick: () => switchTab('signup'), children: "Sign Up" })] }), errorMessage && (_jsx("div", { className: "mb-4 p-3 rounded-lg bg-red-50 border border-red-200 text-red-700 text-sm", children: errorMessage })), providers.includes('github') && (_jsxs("button", { type: "button", className: "w-full flex items-center justify-center gap-3 px-6 py-3 bg-zinc-900 text-white font-medium rounded-xl hover:bg-zinc-800 transition-colors cursor-pointer", onClick: () => handleOAuth('github'), children: [GITHUB_ICON, _jsx("span", { children: activeTab === 'signin' ? 'Sign in with GitHub' : 'Sign up with GitHub' })] })), _jsx("p", { className: "mt-6 text-center text-xs text-zinc-400", children: "By continuing, you agree to our terms of service." })] }));
}
